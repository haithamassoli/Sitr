...
- [x] **M3-T03 Filter builder + live updates** (M) deps: M3-T01, M2-T05 — `FilterBuilder` picks exclude/include per display and installs with `updateContentFilter`, no stream restart; `selftest_filter ok=true`: Off app's marker absent from 44 consecutive frames (0 leaked, including before the plan named it), Off→Blur visible in 419 ms, Blur→Off hidden 296 ms after the rule change, an app launched after start captured 204 ms after its window appears — all inside the 1 s target
...
- [~] **M3-T09 Overlap and attribution check** (S) deps: M3-T05 — `selftest_overlap ok=true`, `docs/behaviour.md`: pre-covers and fail-closed covers clip to the window's visible region (z-order subtraction), person covers do not; no cover ever landed on an Off window (0/19 frames, 0/34 pre-covers). Open leak, needs a Policy decision: attribution uses the detection box CENTRE, so a person in a monitored window with an Off window over their middle resolves to the Off app and gets no cover at all — two upgrade paths in `docs/behaviour.md`
...
- [~] **M4-T10 Robustness** (M) deps: M2-T04, M2-T05 — `SitrCore.Resilience` (backoff capped at 10 s, suspend/resume state machine, mirror dedupe) + `SystemEventMonitor` + `DisplayManager.suspendCapture/resumeCapture` + `Runtime.suspend/resume`; 35 unit tests and `Sitr --selftest robustness --cycles 20` green (23 synthetic cycles, one stream / panel / pipeline per display throughout). Status is `[~]`: the "done when" asks for 20 **real** sleep/wake cycles, which an agent may not run on the owner's machine — the procedure and the log predicate are in `docs/m4/robustness.md` (rows R1–R6 manual, pending; R4/R5 also need a second display). Two-display follow-up (found after the merge, fixed): the resume grace ended on the first stream back and flashed `needsPermission` on every wake — `Runtime.captureFullyBack` now waits for every display; `--selftest robustness --cycles 20` 0 failing cycles across 11 runs (`docs/m4/robustness.md`)
...
- [x] **M5-T03 Release workflow** (M) deps: M5-T01, M5-T02 — `.github/workflows/release.yml` (tag `v*`, draft release, unsigned fallback without secrets) + `CHANGELOG.md`; actionlint clean. Ran end to end on the pushed `v0.1.0` tag (run `34026391713`): tests, Developer ID signing, notarization accepted, staple, DMG, notes from the `[0.1.0]` section, draft release with `Sitr-0.1.0.dmg` + `checksums.txt`. Published; the artefact was then re-downloaded and checked on this Mac — `shasum -c` OK, `spctl -a -vv` `accepted` / `source=Notarized Developer ID`, entitlements `app-sandbox` + `files.user-selected.read-only` only
...
- [~] **M5-T04 Homebrew cask** (S) deps: M5-T03 — `packaging/homebrew-sitr/Casks/sitr.rb` (`ruby -c` clean) + `scripts/bump-cask.sh` + a tag-gated bump step in `release.yml` (actionlint clean, no-op without `HOMEBREW_TAP_TOKEN`); push path exercised against a local stand-in tap. Status is `[~]`: the `homebrew-sitr` tap does not exist yet and `brew audit`/`brew install` need a published notarized DMG, so the "done when" is unmet (`docs/m5/release.md` → Homebrew tap)

...
@Suite struct FailClosedAndRateTests {
    ...
    @Test func captureRateFollowsVisibleCurtainWindowsPerDisplay() {
        ...
        // M4-T06: Low Power caps both rates; on mains power (`cap: .max`) the Curtain rate is untouched.
        #expect(captureFPS(windows: [blur, curtainA], rules: rules, displayID: 1, curtainFPS: 30, cap: 8) == 8)
        #expect(captureFPS(windows: [blur, off], rules: rules, displayID: 1, curtainFPS: 30, cap: 8) == 8)
        #expect(captureFPS(windows: [blur, curtainA], rules: rules, displayID: 1, curtainFPS: 30, cap: .max) == 30)
